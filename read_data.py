...
def load_info(file_name):
    ...
    # Добавление данных из "Тарификация" в info_about_classes -> {"Предметы": {(Предмет): {(Учитель): (Кол-во часов)}}}
            ...
            if row[0] == name_teacher:
                ...
                for hour in row[2:-4]:
                    if hour != "None":
                        ...
                        if row[1] not in info_about_classes[classes_list[c]]["Предметы"]:
    ...
    # Добавление данных из "Тарификация" в info_about_classes -> {"Корпус": (Корпус)}
    index_class = 0
    for name_class in info_about_classes.keys():
        index_building = 0
        for building in mas_data_base[-1][2:-4]:
            if index_class == index_building:
                info_about_classes[name_class]["Корпус"] = building
            index_building += 1
        index_class += 1   
    # Добавление данных из "Тарификация" в info_about_teachers -> {"Кабинеты": {(Корпус): {(Кабинет): (Вместимость)}}
    mas_auditoriums = load_sheet(file_name, "Учителя", "rows")
